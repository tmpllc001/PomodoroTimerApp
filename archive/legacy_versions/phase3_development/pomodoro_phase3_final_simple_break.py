...
# -*- coding: utf-8 -*-
"""
Pomodoro Timer Phase 3 - Final Complete Version with Simple Break Window
Clean Dual Window Design + Statistics Dashboard + Simple Break Window Integration
最終完成版：全機能統合済み + minimal_timer_demo風のさりげない休憩ウィンドウ
"""
...
try:
    from src.widgets.simple_break_window import SimpleBreakWindow
    SIMPLE_BREAK_WINDOW_AVAILABLE = True
    logger = logging.getLogger(__name__)
    logger.info("✅ シンプル休憩ウィンドウモジュール読み込み完了")
except ImportError as e:
    logger = logging.getLogger(__name__)
    logger.warning(f"⚠️ シンプル休憩ウィンドウモジュールが見つかりません: {e}")
    SIMPLE_BREAK_WINDOW_AVAILABLE = False
...
class TaskManager(QObject):
    ...
    def __init__(self):
        super().__init__()
        self.tasks_file = Path("data/tasks_phase3_simple_break.json")
...
class StatisticsManager:
    ...
    def __init__(self):
        self.stats_file = Path("data/stats_phase3_simple_break.json")
...
🎯 Phase 3 完成機能:
   ...
   ✅ Simple Break Window (minimal_timer_demo風)
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        """UI初期化"""
        self.setWindowTitle("🍅 Pomodoro Timer Phase 3 - Final Complete Version with Simple Break")
    ...
    def test_simple_break_window(self):
        ...
        if SIMPLE_BREAK_WINDOW_AVAILABLE:
            self.show_simple_break_window("short", 1)  # 1分の短い休憩でテスト
        else:
            QMessageBox.warning(self, "警告", "シンプル休憩ウィンドウモジュールが利用できません")
    ...
    def show_simple_break_window(self, break_type: str, duration_minutes: int):
        """シンプル休憩ウィンドウ表示"""
        if not SIMPLE_BREAK_WINDOW_AVAILABLE:
        ...
        try:
            ...
            # 新しいシンプル休憩ウィンドウを作成
            self.break_window = SimpleBreakWindow(break_type, duration_minutes)
    ...
    def on_break_window_finished(self):
        ...
        # さりげない通知（minimal_timer_demo風）
        self.statusBar().showMessage("休憩終了 - 次の作業セッションを開始できます", 3000)
    ...
    def refresh_stats_display(self):
        ...
        stats_text = f"""
Phase 3 Final with Simple Break Window - 統計レポート ({datetime.now().strftime('%Y-%m-%d %H:%M:%S')})
...
def main():
    ...
    logger.info("🚀 Pomodoro Timer Phase 3 Final Complete Version with Simple Break Window 起動完了")
    logger.info("✅ 全機能統合済み: Clean Dual Window + Dashboard + Minimal Mode + Simple Break Window")
    if SIMPLE_BREAK_WINDOW_AVAILABLE:
        logger.info("☕ シンプル休憩ウィンドウ機能: 利用可能（minimal_timer_demo風）")
    else:
        logger.warning("⚠️ シンプル休憩ウィンドウ機能: フォールバックモード")
