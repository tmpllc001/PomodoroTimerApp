...
# -*- coding: utf-8 -*-
"""
Pomodoro Timer Phase 3 - Final Complete Version with Break Window
Clean Dual Window Design + Statistics Dashboard + Break Window Integration
最終完成版：全機能統合済み + 休憩ウィンドウ機能追加
"""
...
try:
    from src.widgets.break_window import BreakWindow
    BREAK_WINDOW_AVAILABLE = True
    logger = logging.getLogger(__name__)
    logger.info("✅ 休憩ウィンドウモジュール読み込み完了")
except ImportError as e:
    logger = logging.getLogger(__name__)
    logger.warning(f"⚠️ 休憩ウィンドウモジュールが見つかりません: {e}")
    BREAK_WINDOW_AVAILABLE = False
...
class TimerDataManager(QObject):
    ...
    break_started = pyqtSignal(str, int)  # (break_type, duration_minutes) - 新規追加
...
class TaskManager(QObject):
    ...
    def __init__(self):
        super().__init__()
        self.tasks_file = Path("data/tasks_phase3_break.json")
...
class StatisticsManager:
    ...
    def __init__(self):
        self.stats_file = Path("data/stats_phase3_break.json")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, timer_data: TimerDataManager, task_manager: TaskManager, 
        ...
        self.break_window = None  # 休憩ウィンドウ参照
    ...
    def init_ui(self):
        """UI初期化"""
        self.setWindowTitle("🍅 Pomodoro Timer Phase 3 - Final Complete Version with Break Window")
        ...
        # 休憩ウィンドウテストボタン（開発用）
        test_break_btn = QPushButton("🛌 休憩ウィンドウテスト")
        test_break_btn.clicked.connect(self.test_break_window)
    ...
    def test_break_window(self):
        """休憩ウィンドウテスト表示（開発用）"""
        if BREAK_WINDOW_AVAILABLE:
            self.show_break_window("short", 1)  # 1分の短い休憩でテスト
        else:
            QMessageBox.warning(self, "警告", "休憩ウィンドウモジュールが利用できません")
    
    def on_break_started(self, break_type: str, duration_minutes: int):
        """休憩開始時の処理（新規追加）"""
        self.show_break_window(break_type, duration_minutes)
    
    def show_break_window(self, break_type: str, duration_minutes: int):
        """休憩ウィンドウ表示"""
        if not BREAK_WINDOW_AVAILABLE:
            # フォールバック：通常のメッセージボックス
            break_name = "長い休憩" if break_type == "long" else "短い休憩"
            QMessageBox.information(self, "休憩時間", 
                                  f"🛌 {break_name}の時間です！\n\n"
                                  f"⏰ {duration_minutes}分間休憩しましょう\n"
                                  f"💧 水分補給を忘れずに\n"
                                  f"🤸 軽いストレッチもおすすめです")
            return
        ...
        try:
            ...
            # 新しい休憩ウィンドウを作成
            self.break_window = BreakWindow(break_type, duration_minutes)
            
            # シグナル接続
            self.break_window.break_finished.connect(self.on_break_window_finished)
            self.break_window.skip_break.connect(self.on_break_window_skipped)
    ...
    def on_break_window_finished(self):
        """休憩ウィンドウ終了時の処理"""
        logger.info("✅ 休憩ウィンドウ自然終了")
        self.break_window = None
        
        # 作業セッションの自動開始は行わない（ユーザーが手動で開始）
        QMessageBox.information(self, "休憩終了", "休憩が終了しました！\n次の作業セッションを開始してください。")
    
    def on_break_window_skipped(self):
        """休憩ウィンドウスキップ時の処理"""
        logger.info("⏩ 休憩ウィンドウスキップ")
        self.break_window = None
        
        # 作業セッションの自動開始は行わない
        QMessageBox.information(self, "休憩スキップ", "休憩をスキップしました！\n次の作業セッションを開始してください。")
    ...
    def refresh_stats_display(self):
        ...
        stats_text = f"""
Phase 3 Final with Break Window - 統計レポート ({datetime.now().strftime('%Y-%m-%d %H:%M:%S')})
...
🎯 Phase 3 完成機能:
   ...
   ✅ Complete Task Management
   ✅ Break Window Integration
   ✅ Stretch Guide & Motivational Content
        """.strip()
    ...
    def on_session_completed(self, session_type: str, duration: int):
        ...
        # 通知（作業セッション完了時のみ - 休憩は休憩ウィンドウで処理）
        if session_type == "work":
            session_name = "作業"
            QMessageBox.information(self, "セッション完了", 
                                  f"{session_name}セッションが完了しました！\n\n"
                                  f"🛌 休憩ウィンドウが表示されます\n"
                                  f"💪 お疲れ様でした！")
...
def main():
    ...
    logger.info("🚀 Pomodoro Timer Phase 3 Final Complete Version with Break Window 起動完了")
    logger.info("✅ 全機能統合済み: Clean Dual Window + Dashboard + Minimal Mode + Break Window")
    if BREAK_WINDOW_AVAILABLE:
        logger.info("🛌 休憩ウィンドウ機能: 利用可能")
    else:
        logger.warning("⚠️ 休憩ウィンドウ機能: フォールバックモード")
