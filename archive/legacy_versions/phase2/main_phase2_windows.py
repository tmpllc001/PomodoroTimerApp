...
# -*- coding: utf-8 -*-
"""
Pomodoro Timer Phase 2 - Windows版（実際の音声再生対応）
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('phase2_windows.log', encoding='utf-8'),
...
# Phase 2 機能インポート（Windows版は実際の音声再生を使用）
from features.window_resizer import WindowResizer
from features.statistics import PomodoroStatistics
from features.music_presets_real import MusicPresetsSimple as MusicPresets
...
# 以下、main_phase2.pyと同じコード（PomodoroTimerPhase2クラス）
# インポートだけ変更されています
...
class PomodoroTimerPhase2(QMainWindow):
    ...
    def __init__(self):
        ...
        logger.info("✅ Phase 2 ポモドーロタイマー初期化完了（Windows版）")
        
    def setup_ui(self):
        """UI設定"""
        self.setWindowTitle("🍅 Pomodoro Timer Phase 2 (Windows)")
...
def main():
    """メイン実行"""
    print("🚀 Pomodoro Timer Phase 2 起動中 (Windows版)...")
    logger.info("🚀 Phase 2 アプリケーション開始 (Windows版)")
    ...
    app.setApplicationName("Pomodoro Timer Phase 2 Windows")
    ...
    try:
        ...
        print("✅ Phase 2 起動完了！(Windows版)")
        print("🍅 新機能:")
        print("  - 🪟 ウィンドウサイズ自動制御")
        print("  - 📊 統計機能")
        print("  - 🎵 音楽プリセット（実際の音声再生）")
        
        if AUDIO_AVAILABLE:
            print("\n🔊 音声システム: 有効")
            print("MP3ファイルがあれば実際に音が出ます！")
        else:
            print("\n🔇 音声システム: 無効")
        
        logger.info("✅ Phase 2 アプリケーション初期化完了 (Windows版)")
